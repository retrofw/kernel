/*
 * linux/include/asm-mips/mach-jz4750l/jz4750ldmac.h
 *
 * JZ4750L DMAC register definition
 *
 * Copyright (C) 2008-2014 Ingenic Semiconductor Co., Ltd.
 */
#ifndef __JZ4750LDMAC_H__
#define __JZ4750LDMAC_H__
...
#define DMAC_DSAR(n)		(DMAC_BASE + (n) * 0x20 + 0x00) /* DMA source address */
#define DMAC_DTAR(n)		(DMAC_BASE + (n) * 0x20 + 0x04) /* DMA target address */
#define DMAC_DTCR(n)		(DMAC_BASE + (n) * 0x20 + 0x08) /* DMA transfer count */
#define DMAC_DRSR(n)		(DMAC_BASE + (n) * 0x20 + 0x0C) /* DMA request source */
#define DMAC_DCCSR(n)		(DMAC_BASE + (n) * 0x20 + 0x10) /* DMA control/status */
#define DMAC_DCMD(n)		(DMAC_BASE + (n) * 0x20 + 0x14) /* DMA command */
#define DMAC_DDA(n)		(DMAC_BASE + (n) * 0x20 + 0x18) /* DMA descriptor address */
#define DMAC_DSD(n)		(DMAC_BASE + (n) * 0x04 + 0xC0) /* DMA Stride Address */
...
#define DMAC_DMACKE		(DMAC_BASE + 0x0310) /* DMA clock enable */
...
#define REG_DMAC_DMACKE(m)	REG32(DMAC_DMACKE)
...
#define __dmac_channel_clear_transmit_halt(n)					\
	do {									\
		...
		REG_DMAC_DCCSR(n) &= ~DMAC_DCCSR_HLT;				\
		REG_DMAC_DMACR(n) &= ~DMAC_DMACR_HLT;				\
	} while (0)
...
#define __dmac_channel_clear_address_error(n)					\
	do {									\
		...
		REG_DMAC_DCCSR(n) &= ~DMAC_DCCSR_AR;				\
		REG_DMAC_DMACR(n) &= ~DMAC_DMACR_AR;				\
	} while (0)
...
#define __dmac_channel_set_doorbell(n)	( REG_DMAC_DMADBSR(n) = (1 << (n)))

#define __dmac_channel_irq_detected(n)  ( REG_DMAC_DMAIPR(n) & (1 << (n)))
#define __dmac_channel_ack_irq(n)       ( REG_DMAC_DMAIPR(n) &= ~(1 <<(n)))

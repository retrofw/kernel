...
struct jz47xx_spi_info {
	...
	int			is_pllclk:1;	/* source clock: 1---pllclk;0---exclk */
	unsigned long		board_size;	/* number of the spi devices */
	struct spi_board_info	*board_info; 	/* link to spi devices info */

	u32			num_chipselect;	/* spi_master.num_chipselect */
	u32			*chipselect;

	int			use_dma;	/* spi use DMA */

	/* be defined by spi devices driver user */
	void (*set_cs)(struct jz47xx_spi_info *spi, u8 cs, unsigned int pol);
	/* configure spi function pins (CLK,DR,RT) by user if need. */

    ...
    )
{
    ...
    mdlMap = FindMdlMap(mdl, VIVANTE_PID);

    if (mdlMap == gcvNULL)
    {
        mdlMap = _CreateMdlMap(mdl, VIVANTE_PID);
    ...
    )
{
    ...
    if (Logical)
    {
        ...
        task = FIND_TASK_BY_PID(mdlMap->real_pid);
    ...
    )
{
    ...
    *Physical = (gctPHYS_ADDR) mdl;
	...
	gckOS_Print("gckOS_AllocateNonPagedMemory: "
                "Bytes->0x%x, Mdl->%p, Logical->0x%x dmaHandle->0x%x",
                (gctUINT32)bytes,
                mdl,
                (gctUINT32)mdl->addr,
	...
	gckOS_Print(                "vmaAddr->0x%x pid->%d",
                (gctUINT32)mdlMap->vmaAddr,
                mdlMap->pid);
    ...
    )
{
    ...
    while (mdlMap != gcvNULL)
    {
		...
		gckOS_Print("gckOS_FreeNonPagedMemory: "
                                "Mdl->0x%x Logical->0x%x vmaAddr->0x%x",
                                (gctUINT32)mdl,
                                (gctUINT32)mdl->addr,
                                (gctUINT32)mdlMap->vmaAddr);
    ...
    )
{
    ...
    while (mdl != gcvNULL)
    {
	gckOS_Print("[Get] mdl->addr: 0x%08X\n", mdl->addr);
        ...
        {
		gckOS_Print("[Get] mdl->vmaAddr: 0x%08X\n", mdlMap->vmaAddr);
            if (mdl->dmaHandle)
            {
                /* The memory was from coherent area. */
                *Address = (gctUINT32)mdl->dmaHandle
    ...
    )
{
    ...
    /* Save the process ID. */
	signal->process = (gctHANDLE) VIVANTE_PID;
    ...
    )
{
	...
	if (!VIVANTE_IS_IN_SAMEPROCESS(signal->process))
    {
        gcmkTRACE_ZONE(gcvLEVEL_ERROR,
            gcvZONE_OS,
    ...
    )
{
    ...
    case gcvBROADCAST_GPU_IDLE:
        ...
        gcmkONERROR(gckHARDWARE_QueryPowerManagementState(Hardware, &state));

        /* Query whether the GPU is idle or not. */
        gcmkONERROR(gckHARDWARE_QueryIdle(Hardware, &idle));

        /* Test if the GPU is ON and idle. */
        if ((state == gcvPOWER_ON) && idle)
        {
            /* Switch to ON mode. */
            gcmkONERROR(gckHARDWARE_SetPowerManagementState(Hardware,
                                                            gcvPOWER_IDLE));

            /* Switched to IDLE state. */
            Os->gpuIdle = gcvTRUE;
        }
    ...
    case gcvBROADCAST_GPU_COMMIT:
        if (Os->gpuIdle)
        {
            ...
            /* IDLE to ON state handled. */
            Os->gpuIdle = gcvFALSE;
        }
        break;

    case gcvBROADCAST_GPU_STUCK:
        gcmkTRACE(gcvLEVEL_ERROR, "GPU seems to be stuck!");
        break;
    }

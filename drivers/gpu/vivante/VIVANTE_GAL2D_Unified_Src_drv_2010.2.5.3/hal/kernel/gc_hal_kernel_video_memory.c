    ...
    )
{
	...
	printk("dump allocated Node:\n");
    for (node = Memory->sentinel[Bank].VidMem.next;
         node->VidMem.bytes != 0;
         node = node->VidMem.next)
    {
		printk("\t\tnode->VidMem.physical= 0x%08x offset= %08x bytes=%d\n", 
			   (int)node->VidMem.physical,
			   (int)node->VidMem.offset,
			   (int)node->VidMem.bytes
			);
	}

	printk("dump freeNode:\n");
    for (node = Memory->sentinel[Bank].VidMem.nextFree;
         node->VidMem.bytes != 0;
         node = node->VidMem.nextFree)
    {

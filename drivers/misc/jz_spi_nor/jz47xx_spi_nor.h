...
/**
 * spi_node_write_setup - Fill structure spi_transfer for Flash write.
 * @x:		pointer of spi_transfer structure
 * @spi:	SPI device
 * @tx_buf:	write buffer
 * @tx_dma:	write buffer dma physics address
 * @len:	write data length (Unit: Bytes)
 *
 * NOTE:
 *	set tx_dma 0, this SPI node use PIO mode.
 *	give the DMA physics address, use DMA mode.
 */
static void inline spi_node_write_setup(struct spi_transfer *x, struct spi_device *spi,
					const void *tx_buf, const dma_addr_t tx_dma, int len)
{
	x->rx_buf		= NULL;
	x->tx_buf		= tx_buf;
	x->rx_dma		= (u32)NULL;
...
/**
 * spi_node_read_setup - Fill structure spi_transfer for Flash read.
 * @x:		pointer of spi_transfer structure
 * @spi:	SPI device
 * @rx_buf:	read buffer
 * @rx_dma:	read buffer dma physics address
 * @len:	read data length (Unit: Bytes)
 *
 * NOTE:
 *	set rx_dma 0, this SPI node use PIO mode.
 *	give the DMA physics address, use DMA mode.
 */
static void inline spi_node_read_setup(struct spi_transfer *x, struct spi_device *spi,
					void *rx_buf, dma_addr_t rx_dma, int len)
{
	x->rx_buf		= rx_buf;
	x->tx_buf		= NULL;
	x->rx_dma		= rx_dma;

...
static long jz_nordev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == NOR_WRITE || cmd == NOR_READ) {
		copy_from_user(&jz_args, (void __user *)arg, sizeof(struct jz_nordev_args));
		buf = kzalloc(jz_args.c_size + 32, GFP_KERNEL);
		if (!buf)
			return -ENOMEM;

		/* We need buffer address align to 32Byte */
		data_buf = (u32)buf % 32 ? buf - ((u32)buf % 32) + 32 : buf;
		dev_dbg(&flash->spi->dev, "buf address:\t0x%08x\n32Byte align:\t%d\n", (u32)buf, (u32)buf % 32);
		dev_dbg(&flash->spi->dev, "databuf address:\t0x%08x\n", (u32)data_buf);
	}
	...
	case NOR_GET_INFO:
		...
		memset(&jz_args, 0, sizeof(struct jz_nordev_args));

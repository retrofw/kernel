...
{
	struct nand_chip *chip = mtd->priv;
	unsigned long timeo = jiffies + 2;	//wait 20ms for HZ=100
			 ...
			 struct mtd_oob_ops *ops)
{
	...
	DEBUG(MTD_DEBUG_LEVEL3, "nand_read_oob: from = 0x%08Lx,ooblen= %lld,len:%lld,oobretlen:%lld,ops->mode = %d\n",
...
static uint32_t calc_free_size(struct mtd_info *mtd)
{
	...
	if ((pagesize / eccsize) * eccbytes + eccpos > oobsize)
		freesize = 512;
	else
		freesize = 0;

	printk("==>%s: pagesize=%d, eccsize=%d, eccbytes=%d, eccpos=%d, oobsize=%d\n", __func__, pagesize, eccsize, eccbytes, eccpos, oobsize);
						  ...
						  int busw, int *maf_id)
{
	...
	int tmp_id, tmp_manf;
	uint8_t ext_id[4] = {0};
	uint32_t *ext_id_p = (uint32_t *)ext_id;
	int flash_num;
...
int nand_scan_ident(struct mtd_info *mtd, int maxchips)
{
	...
	printk("==>%s: L%d, global_mafid = 0x%02x\n", __func__, __LINE__, global_mafid);
	if (IS_ERR(type)) {
...
int nand_scan_tail(struct mtd_info *mtd)
{
	...
	printk("==>%s: NAND_MAX_CHIPS = %d\n", __func__, NAND_MAX_CHIPS);
